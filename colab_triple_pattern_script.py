...
包含：
  ...
  2. 🐂 看涨三重底 (Triple Bottom) 7 种经典子形态判定
  3. 🐻 看跌三重顶 (Triple Top) 7 种经典子形态判定
  4. 🎯 TFLab 风格入场 (Entry)、止损 (SL)、第一目标 (TP1 1:1)、第二目标 (TP2 1.618)、盈亏比 (R:R) 计算
  5. 多周期扫描 (日线 1d, 周线 1w, 月线 1mo, 4小时 4h, 60m 等)
  6. 自动导出与下载 CSV 文件，与主平台格式 100% 兼容
"""
...
def generate_colab_script_for_tickers(tickers: list[str], pool_name: str = "系统品种库", selected_tfs: list[str] = None) -> str:
    ...
    script = f'''# ==============================================================================
# 🚀 Google Colab · 🌟「三重底 & 三重顶」双向形态极速扫描脚本
# 股票池来源: {pool_name} (共 {len(tickers)} 支品种)
# 扫描周期: {', '.join(selected_tfs)}
# 扫描模式: 🐂 看涨三重底 (Triple Bottom) + 🐻 看跌三重顶 (Triple Top) 双向全量扫描
# ==============================================================================
# 使用方法：
...
# ------------------------------------------------------------------------------
# 🧠 三重顶底识别算法核心 (基于 Al Brooks 价格行为学 & TFLab 几何投影)
# ------------------------------------------------------------------------------
...
def classify_triple_bottom(low1, low2, low3, mid_high_12, mid_high_23, broke_support_23, flat_tol=0.02):
    ...
    if flat_all:
        return "完美三重底 (Perfect Triple Bottom)", 0.92, f"三低点差异 {{max(d12, d23):.2%}}，接近持平"
    if low2 < low1 and low2 < low3 and d12 > flat_tol and d23 > flat_tol:
        ...
        return "头肩底/截断楔形 (Head & Shoulders Bottom)", conf, f"中间头部更低({{low2:.2f}})，双肩对称"
    if ascending and broke_support_23:
        return "抬高双底失败突破型 (Failed BO below HL DB)", 0.85, "低点逐步抬高，第3次回踩假破拉回"
    if broke_support_23:
        if d23 <= flat_tol * 2:
            return "双底回调型 (Double Bottom Pullback)", 0.80, "双底确立后回踩确认支撑"
        else:
            return "双底跌破失败型 (Failed BO below DB)", 0.80, "短暂刺穿支撑后多头强力收复"
    if descending and not flat_all:
        return "楔形三重底 (Wedge Bottom)", 0.75, f"低点依次降低 (L1>L2>L3)，下行动能衰竭"
    if ascending and mid_high_12 > mid_high_23:
        return "三角形三重底 (Triangle Bottom)", 0.75, "低点抬高同时高点走低，收敛蓄势"
    return "未分类三次探底 (Unclassified Triple Bottom)", 0.55, "满足三次探底特征"
...
def classify_triple_top(high1, high2, high3, mid_low_12, mid_low_23, broke_resist_23, flat_tol=0.02):
    ...
    if flat_all:
        return "完美三重顶 (Perfect Triple Top)", 0.92, f"三高点差异 {{max(d12, d23):.2%}}，接近持平"
    if high2 > high1 and high2 > high3 and d12 > flat_tol and d23 > flat_tol:
        shoulder_diff = _pct_diff(high1, high3)
        conf = 0.88 if shoulder_diff < 0.03 else 0.72
        return "头肩顶/截断上升楔形 (Head & Shoulders Top)", conf, f"中间头部更高({{high2:.2f}})，双肩对称"
    if descending and broke_resist_23:
        return "降低双顶失败突破型 (Failed BO above LH DT)", 0.85, "高点逐步降低，第3次冲高遇阻回落"
    if broke_resist_23:
        if d23 <= flat_tol * 2:
            return "双顶回调型 (Double Top Pullback)", 0.80, "双顶确立后反弹确认阻力"
        else:
            return "双顶突破失败型 (Failed BO above DT)", 0.80, "短暂刺穿阻力后空头强力打压"
    if ascending and not flat_all:
        return "上升楔形三重顶 (Rising Wedge Top)", 0.75, f"高点依次抬高 (H1<H2<H3)，上行动能衰竭"
    if descending and mid_low_12 < mid_low_23:
        return "下降三角形三重顶 (Descending Triangle Top)", 0.75, "高点走低同时支撑平齐，向下破位"
    return "未分类三次探顶 (Unclassified Triple Top)", 0.55, "满足三次探顶特征"
...
def scan_patterns(df, symbol="", period="1d", swing_window=3, lookback_bars=150, max_spacing=80, min_spacing=3, break_tol=0.01, flat_tol=0.02):
    ...
    if len(swing_low_idx) >= 3:
        for a in range(len(swing_low_idx) - 2):
            ...
            if not (min_spacing <= (i2 - i1) <= max_spacing and min_spacing <= (i3 - i2) <= max_spacing):
                continue
            ...
            if has_broken_sup:
                status = "invalidated"
                reason = f"已失效：跌破支撑 {{lowest_point:.2f}}"
            elif bars_since > 45:
                status = "expired"
                reason = f"已过期：后置 {{bars_since}} 根未突破"
            elif has_broken_neck:
                status = "confirmed"
                reason = f"已突破：收盘站上颈线 {{neckline:.2f}}"
            else:
                status = "active"
                reason = "形态成型中，观察是否突破颈线"
    ...
    if len(swing_high_idx) >= 3:
        for a in range(len(swing_high_idx) - 2):
            ...
            if has_broken_res:
                status = "invalidated"
                reason = f"已失效：突破阻力 {{highest_point:.2f}}"
            elif bars_since > 45:
                status = "expired"
                reason = f"已过期：后置 {{bars_since}} 根未破位"
            elif has_broken_neck:
                status = "confirmed"
                reason = f"已跌破：收盘跌破颈线 {{neckline:.2f}}"
            else:
                status = "active"
                reason = "形态成型中，观察是否跌破颈线"
...
# ------------------------------------------------------------------------------
print(f"\\n" + "="*80)
print(f"🚀 开始「🌟 三重顶底双向极速扫描」 | 股票池: {{len(SCAN_TICKERS)}} 支品种 | 周期: {', '.join(selected_tfs)}")
print("="*80)
...
if all_results:
    ...
    cols = [
        ...
        "idx1", "idx2", "idx3", "p1", "p2", "p3", "neckline",
        "entry_price", "stop_loss", "tp1", "tp2", "risk_reward",
        "note", "status", "status_reason", "bars_since_p3", "latest_close", "scan_time"
    ]
