        ...
        with st.expander("⭐ 重点关注品种告警汇总", expanded=bool(starred_set)):
            if not starred_set:
                ...
                with st.expander("↕️ 调整重点关注品种显示顺序", expanded=False):
                    col_hint, col_clear = st.columns([3, 1.5])
                    with col_hint:
                        st.caption("💡 点击下方的 **[ ⬆️ 上移 ]** 或 **[ 🔻 下移 ]** 按钮调整顺序：")
                    with col_clear:
                        if st.button("🗑️ 批量取消全部重点关注品种", key="btn_clear_all_starred_reorder", use_container_width=True):
        ...
        if df.empty:
            ...
            try:
                ...
                for idx, row in df.iterrows():
                    ...
                    now_t = str(int(_t_mod.time() * 1000))
