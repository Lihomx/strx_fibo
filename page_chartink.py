...
def render_page_chartink():
    ...
    def _on_chartink_group_change():
        ...
        if sel and sel != "— 选择载入分组 —":
            ...
            if "全部分组合并" in sel:
                ...
                st.session_state["chartink_tickers"] = " ".join(unique_tks)
    ...
    with st.expander("☁️ Google Colab 算力扫描渠道 (全美股 / 全A股 / 全部分组合并 极速 4H 突破扫描与结果导入)", expanded=False):
        ...
        with colab_c1:
            ...
            saved_pool_chartink = cfg.get("colab_selected_pool_chartink", "")
            pool_def_idx = 0
            if saved_pool_chartink and saved_pool_chartink in pool_options:
                pool_def_idx = pool_options.index(saved_pool_chartink)

            def _on_chartink_colab_pool_change():
                p_val = st.session_state.get("chartink_colab_selected_pool")
                if p_val:
                    storage.save_config({"colab_selected_pool_chartink": p_val})
            ...
            with p_col1:
                selected_pool = st.selectbox(
                    ...
                    on_change=_on_chartink_colab_pool_change,
            ...
            if "自定义勾选多个分组" in selected_pool:
                ...
                saved_grps = cfg.get("colab_custom_groups_chartink")
                ...
                if "chartink_colab_custom_groups" not in st.session_state:
                    st.session_state["chartink_colab_custom_groups"] = valid_defaults

                # 快捷全选 / 清空 / 常用 辅助操作栏
                q_c1, q_c2, q_c3, _ = st.columns([1, 1, 1.2, 1.8])
                with q_c1:
                    if st.button("➕ 全选所有分组", key="btn_select_all_chartink", use_container_width=True):
                        st.session_state["chartink_colab_custom_groups"] = list(all_grp_names)
                        storage.save_config({"colab_custom_groups_chartink": list(all_grp_names), "colab_custom_groups": list(all_grp_names)})
                        st.rerun()
                with q_c2:
                    if st.button("🧹 清空所有勾选", key="btn_clear_all_chartink", use_container_width=True):
                        st.session_state["chartink_colab_custom_groups"] = []
                        storage.save_config({"colab_custom_groups_chartink": [], "colab_custom_groups": []})
                        st.rerun()
                with q_c3:
                    a_us_names = [gn for gn in all_grp_names if any(k in gn for k in ["美股", "标普", "纳斯达克", "A股", "沪深", "中证"])]
                    if st.button("🎯 勾选主要股市", key="btn_main_markets_chartink", use_container_width=True, help="一键快速勾选美股与A股各核心板块分组"):
                        st.session_state["chartink_colab_custom_groups"] = list(a_us_names)
                        storage.save_config({"colab_custom_groups_chartink": list(a_us_names), "colab_custom_groups": list(a_us_names)})
                        st.rerun()

                def _on_chartink_colab_custom_groups_change():
                    chosen = st.session_state.get("chartink_colab_custom_groups", [])
                    storage.save_config({"colab_custom_groups_chartink": chosen, "colab_custom_groups": chosen})

                selected_custom_groups = st.multiselect(
                    "勾选要合并扫描的分组 (可任意多选，系统自动去重合并并持久记忆)",
                    options=all_grp_names,
                    key="chartink_colab_custom_groups",
                    on_change=_on_chartink_colab_custom_groups_change,
                ...
                if selected_custom_groups != (saved_grps if saved_grps is not None else []):
                    storage.save_config({"colab_custom_groups_chartink": selected_custom_groups, "colab_custom_groups": selected_custom_groups})
    ...
    # ── 过滤、搜索与分页控制栏 ──────────────────────────────────────
    col_f1, col_f2, col_f3 = st.columns([1.8, 1.2, 1.0])
    with col_f1:
        search_q = st.text_input("🔍 搜索品种代码", key="chartink_search_q", placeholder="输入代码如 AAPL, TSLA...")
    with col_f2:
    ...
    with col_pass_hdr1:
        st.markdown(f"### ✅ 通过筛选的品种 ({total_passed_cnt} 支)")
    with col_pass_hdr2:
        if filtered_passed:
            if st.button("⭐ 批量收藏当前筛选品种", key="chartink_fav_all_passed", use_container_width=True):
                added_cnt = 0
    ...
    if not filtered_passed:
        if passed and search_q:
            st.markdown('<div class="n-warn">未找到符合搜索条件的品种。</div>', unsafe_allow_html=True)
        else:
        ...
        if page_size == "全部":
            ...
            if "chartink_page_idx" not in st.session_state:
                st.session_state["chartink_page_idx"] = 1
            if st.session_state["chartink_page_idx"] > total_pages:
                st.session_state["chartink_page_idx"] = total_pages
            if st.session_state["chartink_page_idx"] < 1:
                st.session_state["chartink_page_idx"] = 1
                
            curr_page_idx = st.session_state["chartink_page_idx"]
            
            if total_pages > 1:
                col_p1, col_p2, col_p3 = st.columns([1, 2, 1])
                with col_p1:
                    if st.button("⬅️ 上一页", key="ci_prev_p", disabled=(curr_page_idx <= 1), use_container_width=True):
                        st.session_state["chartink_page_idx"] = max(1, curr_page_idx - 1)
                        st.rerun()
                with col_p2:
                ...
                with col_p3:
                    if st.button("下一页 ➡️", key="ci_next_p", disabled=(curr_page_idx >= total_pages), use_container_width=True):
                        st.session_state["chartink_page_idx"] = min(total_pages, curr_page_idx + 1)
                        st.rerun()
    ...
    if failed:
        with st.expander(f"❌ 未通过品种（共 {len(failed)} 个）", expanded=False):
            ...
            if failed_tickers:
                sel_f_tk = st.selectbox("🔍 选择查看未通过品种的 7 条规则检测值", options=["— 请选择品种 —"] + failed_tickers, key="ci_failed_inspect_picker")
