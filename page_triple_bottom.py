...
def render_triple_bottom_page():
    ...
    with st.expander("☁️ Google Colab 算力扫描渠道 (全美股 / 全A股 极速扫描与结果导入)", expanded=False):
        ...
        with colab_c1:
            ...
            saved_pool_tb = cfg.get("colab_selected_pool_tb", "")
            pool_def_idx = 0
            if saved_pool_tb and saved_pool_tb in pool_options:
                pool_def_idx = pool_options.index(saved_pool_tb)

            def _on_tb_colab_pool_change():
                p_val = st.session_state.get("tb_colab_selected_pool")
                if p_val:
                    storage.save_config({"colab_selected_pool_tb": p_val})
            ...
            with p_col1:
                selected_pool = st.selectbox(
                    ...
                    on_change=_on_tb_colab_pool_change,
